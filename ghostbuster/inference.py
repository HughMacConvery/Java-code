...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        dist = DiscreteDistribution()
        allP = self.allPositions
        for i in range (0, len(allP)):
            position = self.allPositions[i]
            probability = self.getObservationProb(observation, pacPosition, position, jailPosition)
            dist[position] = probability * self.beliefs[position]
        dist.normalize()
        self.beliefs = dist
        "raiseNotDefined()"
    def elapseTime(self, gameState):
        ...
        for i in range(0, len(allP)):
            ...
            for newPos in newPosDist.keys():
                dist[newPos] = prob * newPosDist[newPos] + dist[newPos]
        dist.normalize()
        self.beliefs = dist
        "raiseNotDefined()"
    def getBeliefDistribution(self):
